...
/* Indices used */
CREATE INDEX ON person(UPPER(person_name));

CREATE INDEX ON category(UPPER(category_name));

/***** Office hours testing *****/

CREATE INDEX ON products_in_cart(product_id);

CREATE INDEX ON product(UPPER(product_name));
CREATE INDEX ON product(category_id);
CREATE INDEX ON shopping_cart(person_id);
CREATE  INDEX ON state(UPPER(state_name));
